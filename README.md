...
执行进度在原生对话窗口中显示，不再向消息渠道自动发送等待提示。`/status` 可查询阶段、耗时和可用的运行 ID。中断、明确超时、连接错误、限流、认证失败分别报告；没有证据时不会把 aborted 当作超时，也不会自动重试可能产生副作用的任务。
...
项目默认打开「对话」：选择消息入口与会话，按时间连续阅读输入、工作过程和回复。桌面输入沿用所选会话；「新对话」在下一次发送时建立独立上下文。不同入口不合并。桌面发起的文字回复保存在本机对话，不自动转发到消息渠道；原渠道发起的消息仍正常回传。

运行中每秒刷新，空闲时每 3 秒刷新。展开工作过程可查看后端阶段、工具类别、Codex 进展说明和时间；回复片段随生成更新，审批、回答和停止在当前轮处理。长时间没有事件时明确显示距上次活动的时间，不推断后端内部原因。运行中新的普通消息暂不排队，完成或停止后可继续同一会话。

新消息的过程随每轮持久保存，每轮最多保留最近 1000 条事件，每条文本最多 8000 字符；发生截断时显示提示。回复正文仍完整保存。旧记录仅能展示原来保留的诊断，不补造历史过程。工具原始参数、输出和模型内部推理不写入过程记录。窗口的「记录」保留任务归档、结果重发和最近诊断。

### 任务历史保留

每个入口常用状态保留最近最多 500 条任务、8 MiB 完整内容；执行或发送中的任务不裁剪。旧任务先按月写入私有 `stateDir/archives/`，成功后才从常用状态移出。归档不会自动删除，占用磁盘仍随历史增长，但不参与每次状态轮询与整份状态重写。原生「对话 → 记录 → 归档历史」每页读取 25 条，可查看完整输入和结果、重发未确认送达的结果。请将归档目录与状态一起备份。

常规轮询只返回任务摘要（输入 400 字符、结果 800 字符），对话窗口自动按需读取完整消息，点「加载更早的消息」可读取所选会话的归档。记录页面仍可点击「查看完整任务」。审批使用 10 位短 ID，原生界面展示请求内容与问题选项，并保留完整请求详情。
