...
describe('Codex route ownership and lifecycle', () => {
  it('starts a sandboxed thread, rejects duplicate/busy prompts, returns only final output', async () => {
    ...
    expect(s.rpc.request).toHaveBeenCalledWith('thread/start', expect.objectContaining({ cwd: '/workspace', sandbox: 'workspace-write', approvalPolicy: 'on-request' }))
...
it('keeps the route available when a resumed session has another writer and accepts /new',async()=>{
  const {SessionInUseError}=await import('../src/backends/jsonrpc.js')
  const s=setup('/workspace','thread-1')
  s.rpc.request.mockRejectedValueOnce(new SessionInUseError())
  await s.router.receive(s.message('hello'))
  expect(s.send).toHaveBeenCalledWith(expect.stringContaining('Send /new'))
  expect(s.router.snapshot().closed).toBe(false)
  expect(s.rpc.close).not.toHaveBeenCalled()
  await s.router.receive(s.message('/new'))
