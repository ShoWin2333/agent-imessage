...
This directory is the **DSH plugin** product line, maintained in parallel with the macOS App / Gateway at the repository root. It runs inside DeepSeek Harness Web on **Windows, Linux, and macOS**. The root Agent iMessage app is a separate native macOS product; the two do not share configuration or process lifetime.
...
The unqualified `dsh-imessage` package on npm refers to the upstream release and may not contain this fork's enhancements. To use the image, file, voice, and multi-route features from this repository, build and install the plugin package locally.

macOS / Linux:

```sh
git clone https://github.com/ShoWin2333/dsh-imessage.git
cd dsh-imessage/apps/dsh-imessage
